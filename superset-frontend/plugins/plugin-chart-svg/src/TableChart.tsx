...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  //   useEffect(() => {
  //       fetch(schemeFloor) // "map.svg" is in the public directory
  //         .then((response) => {
  //           if (!response.ok) {
  ...
  //           const data1 = [
  //             { name: 'Кухня', value: 35 },
  //             { name: 'Терасса', value: 48 },
  //             { name: 'Прихожая', value: 15 },
  //             { name: 'Спальня 1', value: 60 },
  //             { name: 'Гостиная', value: 30 },
  //             { name: 'Спальня 2', value: 83 },
  //             { name: 'Спальня 3', value: 95 },
  //           ];
  //           const options = getOptions(data1);
  ...
  const ref = useRef(null);
  const refTolltip = useRef(null);

  const data1 = [
    { name: '1', status: 'Подбор' },
    { name: '2', status: 'Маркетинговая сделка' },
    { name: '3', status: 'Бронь' },
    { name: '4', status: 'Маркетинговая сделка' },
    { name: '5', status: 'Бронь' },
    { name: '6', status: 'Подбор' },
    { name: '7', status: 'Продано' },
    { name: '8', status: 'Маркетинговый резерв' },
    { name: '9', status: 'Маркетинговый резерв' },
    { name: '10', status: 'Подбор' },
    { name: '11', status: 'Проверка' },
    { name: '12', status: 'Сделка в работе' },
    { name: '13', status: 'Подбор' },
    { name: '14', status: 'Бронь' },
    { name: '15', status: 'Бронь' },
    { name: '16', status: 'Подбор' },
    { name: '17', status: 'Подбор' },
  ];

  useEffect(() => {
    // @ts-ignore
    const arrElements = ref.current.querySelectorAll('[name]');
    // @ts-ignore
    ...
    if (arrElements.length) {
      ...
      arrElements.forEach((el: any) => {
        ...
        el.addEventListener('mousemove', (e: any) => {
          ...
          const target = data1.find(item => item.name === name);
          // @ts-ignore
          const backgroundColor = getColor(target.status);
          // @ts-ignore
      ...
      data1.forEach(el => {
        // @ts-ignore
        const index = arrNames.indexOf(el.name);
  ...
  function getColor(status: string) {
    switch (status) {
      ...
      case 'Сделка в работе': {
        return '#8431c5';
      }
      case 'Маркетинговая сделка': {
        return '#ff0303';
      }
      case 'Маркетинговый резерв': {
        return '#67dde0';
      }
      default: {
        return 'transparent';
  ...
  return (
    <div>
      {showScheme ? (
        ...
        >
          <SchemeFloor ref={ref} />
          {/* <div className="floor_widget" ref={ref} id='beef'></div> */}
          <div
            className="tooltip"
            ref={refTolltip}
            style={{
